...
- **Booking Requests / Bookings**
  ...
  - **Flutter API layer (no UI yet)**: `lib/models/booking_issue.dart` (`BookingIssue`, `kindsFor`, `isReportWindowOpen`) and `lib/services/booking_issue_service.dart` wrap the two RPCs plus `listForBooking` / `listForBuilding` / `countOpen`. Report/resolve screens ship in follow-up PRs.
